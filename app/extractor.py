        ...
        try:
            for pg_idx in range(chunk_start, chunk_end):
                ...
                for raw_line in joined:
                    ...
                    # text merging (e.g. 'FSEBC' instead of 'F SEBC'). We also omit \b on the left
                    # because Linux can merge the name as well (e.g. 'LAXMANFSEBC').
                    gms = list(re.finditer(r"([MF])(?=\s|SEBC|OBC|OPEN|NTC|NTD|NTA|NTB|SC|ST|VJ|EWS|SBC|EM|HA|PWD|DEF|PH|ORPHAN|MKB|MINO|D1|D2|D3)", middle))
                    if not gms:
                        continue
                    last_gm = gms[-1]
